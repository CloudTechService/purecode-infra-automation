...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Find instances that can be stopped (running state)
        instances_to_stop = [id for id in instance_ids if instance_states.get(id) == 'running']
        ...
        if instances_to_stop:
            try:
                ...
                results['successful'] = [instance['InstanceId'] for instance in response['StoppingInstances']]
                print(f"Successfully stopped instances: {results['successful']}")
            except ClientError as e:
                error_msg = f"Failed to stop instances: {str(e)}"
                results['errors'].append(error_msg)
                print(f"ERROR: {error_msg}")
        
        # Track instances that were skipped
        for id in instance_ids:
            if id not in instances_to_stop:
