...
def lambda_handler(event, context):
    """
    Lambda function to start RDS instances
    """
            ...
            try:
                ...
                if current_status == 'stopped':
                    try:
                        ...
                        print(f"Successfully initiated start for DB instance: {db_id}")
                    except ClientError as e:
                        error_code = e.response['Error']['Code']
                        if error_code == 'InvalidDBInstanceState':
                            skip_msg = f"{db_id} (cannot be started in current state)"
                            results['skipped'].append(skip_msg)
                            print(f"Skipped: {skip_msg}")
                        else:
                            error_msg = f"Failed to start {db_id}: {str(e)}"
                            results['errors'].append(error_msg)
                            print(f"ERROR: {error_msg}")
                elif current_status in ['available', 'starting']:
                    skip_msg = f"{db_id} (current status: {current_status})"
                    results['skipped'].append(skip_msg)
                    print(f"Skipped: {skip_msg}")
                else:
                    skip_msg = f"{db_id} (current status: {current_status}, cannot be started)"
                    results['skipped'].append(skip_msg)
                    print(f"Skipped: {skip_msg}")
